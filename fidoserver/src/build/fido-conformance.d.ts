/**
 *  + ----------------------------- +
 *  |       FIDO Conformance        |
 *  + ----------------------------- +
 *
*/
export declare const fidoConformanceRouter: import("express-serve-static-core").Router;
export declare const fidoRouteSuffix = "/fido";
/**
 * MDS3ROOT
 *
 * Downloaded from https://mds3.certinfra.fidoalliance.org/
 *
 * Valid until 2045-01-31 @ 00:00 PST
 *
 * SHA256 Fingerprint
 * 66:D9:77:0B:57:71:10:9B:8D:83:55:7B:A2:7D:58:9B:56:BD:B3:BF:DB:DE:A2:D2:42:C4:CA:0D:57:70:A4:7C
 */
export declare const MDS3ROOT = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n";
